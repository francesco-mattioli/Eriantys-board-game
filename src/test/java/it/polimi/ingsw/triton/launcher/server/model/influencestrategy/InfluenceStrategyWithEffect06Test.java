...
class InfluenceStrategyWithEffect06Test {
    private Player p1;
    private Player p2;
    ...
    /**
     * Checks if the influence is calculated correctly without considering the towers.
     */
    @Test
    void testCalculateInfluenceWithoutTowers() {
        try {
            p1.getSchoolBoard().moveTowerOntoIsland(island.getId());
        } catch (EndGameException e) {
            throw new RuntimeException(e);
        }
        assertEquals(2, new InfluenceStrategyWithEffect06().execute(p1, professors, p1, island));
    }
}